...
class System_site{
	...
	public:
		vector <int> vneighbors;						//Vector of neighbors of the site when non periodic conditions
		//int *vneighbors;							//Vector of neighbors of the site when periodic conditions
		int num_neighbors;							//Number of neighbors for the case of non periodic boundary conditions
		int *num_neighbor_states;					//Vector of states of the neighbors of the site to calculate its fitness dued to mutualism
		int num_active_neighbors;					//Number of ocupated neighbors
		int state;									//State of the site: Active (0,1,2...) or Inactive(-1)
		
		double own_wealth;							//Basic wealth
...
class System{
	...
	//const int num_neighbors;						//Number of neighbors of each site. In the case of periodic conditions

	//Dynamics variables
	const int num_states;							//Number of possible states of each site
	const double p_swap;								//Probability of swap with neighbor
	//const double p_difusse;						//Probability of diffuse in an empty neighbor site
	const double own_wealth_value;					//Fixation value for own_wealth
	const int initial_width;							//Initial width of bacteria
	double **Aij;									//Mutualism matrix, each aij determines the benefith of i with j neighbor
	
	vector <int> list_empty_front_sites;				//List of empty sites near the front that can be occupied
	vector <int> list_active_front_sites;			//List of active sites near the front that can reproduce itselfs
	...
	void add_num_neighbor_states(int index, int istate);						//Add an state to the vector of states of a site
	void remove_num_neighbor_states(int index, int istate);					//Removes an state to the vector of states of a site
	...
	bool check_toinactive_front(int site);									//Check if the active site in the front would not be at the front when reproduction happens
	bool check_toactive_front(int site);										//Check if the inactive site would be at the front when reproduction happens
	...
	double mututalistic_term(int site, int site_state,int neighbor_state);	//Computes mutualistic term of a site
	...
	int num_active_sites;							//Number of active sites close to an active site
	
	//Contructor of the class
	System(int dim, int L, int nw, int Ns, double **aij, double pswap, int initial_w, double basic_wealth): 
	dimension(dim),linear_size(L),neighbors_width(nw),num_sites(pow(linear_size,dim)), num_states(Ns), p_swap(pswap), initial_width(initial_w),own_wealth_value(basic_wealth)
	{
	...
	int get_dimension();											//Gives the dimension of the system
	int get_linear_size();										//Gives the linear size of the system
	...
	void initialize_list_active_sites();							//Initialize active sites close to active sites
			...
			for(int jsite=0;jsite<linear_size;jsite++){
				...
				for(int ksite=1;ksite<=neighbors_width;ksite++){
					...
					for(int lsite=isite-ksite;lsite<=isite+ksite;lsite++){
						...
						row=lsite;
						column=jsite+ksite;
						if((row>=0)&&(row<linear_size)) list_sites[site].vneighbors.push_back(((lsite+linear_size)%linear_size)*linear_size+(jsite+ksite+linear_size)%linear_size);
					}
					...
					for(int lsite=jsite-ksite+1;lsite<=jsite+ksite-1;lsite++){
						row=isite+ksite;
						column=lsite;
						if((row>=0)&&(row<linear_size)) list_sites[site].vneighbors.push_back(((isite+ksite+linear_size)%linear_size)*linear_size+(lsite+linear_size)%linear_size);
					}
				...
				list_sites[site].num_neighbors=list_sites[site].vneighbors.size();
...
//Sets states initially in a flat front with initial_width of random states
...
//Introduce empty sites close the front in list_empty_sites
...
void System::initialize_num_neighbor_states(){
	...
	for(int i=0;i<linear_size;i++){
		for(int k=0;k<initial_width;k++){
			...
			for(int j=0;j<list_sites[site].num_neighbors;j++){
...
void System::get_mutualistic_wealth(int istate,int index){
	...
	for(int j=0;j<num_states;j++){
		...
		cout<<index<<": "<<list_sites[index].num_neighbor_states[j]<<" -> "<<list_sites[index].mutualistic_wealth<<endl;
	}
}
...
int System::get_reproduce_site(gsl_rng *r,int empty_site){
	...
	for (int i=0;i<list_sites[empty_site].num_neighbors;i++){
	...
	for(int i=0;i<list_sites[empty_site].num_neighbors;i++){
		...
		cout<<neighbor<<" ("<<list_sites[neighbor].mutualistic_wealth<<") ";
		if((rand_num>sum_ant)&&(rand_num<sum)){
...
void System::reproduce(int empty_site, int reproduce_site){
	...
	for(int i=0;i<list_sites[empty_site].num_neighbors;i++){
		...
		if(neighbor_state>=0){
			...
			list_sites[neighbor].num_active_neighbors++;
			if(position_empty_list[neighbor]<0) add_empty_site(neighbor); //If it is an empty site it is added to the list of empty sites near the front!
		}
	}
...
bool System::check_toinactive_front(int site){
	...
	if(list_sites[site].num_active_neighbors==list_sites[site].num_neighbors) return true;
	else return false;
}
...
void System::swap(gsl_rng *r){
	...
	if(gsl_rng_uniform(r)<p_swap){
		...
		site2=list_sites[site1].vneighbors[gsl_rng_uniform_int(r,list_sites[site1].num_neighbors)];
		
		state1=list_sites[site1].state;
		state2=list_sites[site2].state;
		
		//If it does->swaps site1 with site2
		if(state2>=0){
			...
			if(state1!=state2){
				...
				for(int i=0;i<list_sites[site1].num_neighbors;i++){
				...
				for(int i=0;i<list_sites[site2].num_neighbors;i++){
		...
		//else... TO DO!!!! This would be diffusion to an empty site
	}
}
