...
void System::reproduce(int empty_site, int reproduce_site){
	...
	for(int i=0;i<num_neighbors;i++){
		...
		if(neighbor_state>=0) update_neighbor_reproduction(neighbor,neighbor_state,new_state);
		else{
...
void System::swap(gsl_rng *r){
	...
	if(gsl_rng_uniform(r)<p_swap){
		...
		empty_site=get_empty_site(r);
		do{
			irow=list_sites[empty_site].row+(gsl_rng_uniform_int(r,2*neighbors_width+1)-(neighbors_width+1));
			irow=(irow+linear_size)%linear_size;
			icolumn=list_sites[empty_site].column-(gsl_rng_uniform_int(r,neighbors_width+1));
			icolumn=(icolumn+linear_size)%linear_size;
			site1=irow*linear_size+icolumn;
		...
		do{
			...
			icolumn=list_sites[empty_site].column-(gsl_rng_uniform_int(r,neighbors_width+1));
			icolumn=(icolumn+linear_size)%linear_size;
			site2=irow*linear_size+icolumn;
