...
class System_site{
	...
	public:
		vector <int> vneighbors;								//Vector of neighbors of the site
		int num_neighbors;
...
class System{
	...
	dimension(dim),linear_size(L),neighbors_width(nw),num_sites(pow(linear_size,dim)), num_states(Ns), p_swap(pswap), initial_width(initial_w)
	{
...
void System::get_mutualistic_wealth(int istate,int index){
	...
	for(int j=0;j<num_states;j++){
		...
		cout<<list_sites[index].state<<": "<<j<<"->> "<<list_sites[index].num_neighbor_states[j]<<" -> "<<list_sites[index].mutualistic_wealth<<endl;
	}
}
